...
zaddr_valid = find (TRIOS_ROV.QPAR > 1.0e-5 & ~isnan(TRIOS_ROV.QPAR) & ~isnan(zLL_ROV) );
figure; hold on; box on
plot (zLL_ROV(zaddr_valid),(log10(TRIOS_ROV.QPAR(zaddr_valid))), 'ksq', 'MarkerFaceColor', 'b');
ylabel('log10(QPAR)')
xlabel('LL1+LL2')
% plot (log10(TRIOS_ROV.I1f), zLL1_ROV, 'ksq', 'MarkerFaceColor', 'b');

x = zLL_ROV(zaddr_valid);
y = log10(TRIOS_ROV.QPAR(zaddr_valid));
[p,s] = polyfit (x, y, 1)
...
% root-mean-square-error des valeurs de QPAR (µE/m2/s) retrouvées à partir de LL1 et LL2
